<template>
	<div class="box">
		<Breadcrumb class="crumb" separator=">">
			...
			<BreadcrumbItem>门店退货查询</BreadcrumbItem>
		</Breadcrumb>
		<div class="thisName">
			<span>门店退货查询</span>
		</div>
		...
		data() {
			return {
				...
				columns_detail:[{
						...
						{
							title: 'wBaseDepotReturnId',
							key: 'wBaseDepotReturnId',
							align: 'center',
							width:100
						},
						{
				...
				tableInfo: {
					name: '门店进货查询',
					search: 0,
					url: '/stock/return_order_list?userId=1&start=1&length=10',
					columns: [{
							title: 'wBaseDepotReturnId',
							key: 'wBaseDepotReturnId',
							align: 'center',
						...
						{
							title: 'baseDepotReturnDetailVos',
							key: 'baseDepotReturnDetailVos',
							align: 'center',
							width: 100
						},
						{
...
//				row.storeStockOrderId
				this.$http({
					method: 'get',
					url: window.http + '/stock/return_detail_list?wBaseDepotReturnId='+row.wBaseDepotReturnId,
				}).then((res) => {
					console.log(res)
					if(res.status == 200) {
