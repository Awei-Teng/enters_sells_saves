    ...
    export default {
        ...
        data () {
            return {
            	...
            	tableInfo: {
					name: '巡店查询',
					search: 0,
					url: window.http + '/sell_record/sell_record_list',
					arg:{
					    "start":"1",
					    "length":"20",
					    "storeId":null,
	                        ...
	                        render: (h, params) => {
	                            return h('div', [
	                                h('Button', {
	                                    ...
	                                    on: {
	                                        click: () => {
	                                        	this.now_id = params.row.sellRecordId
	                                        	this.show_detail()
        ...
        methods:{
        	...
        	show_detail(){
        		...
        		this.$http({
					method: 'get',
					url: window.http + '/sell_record/sell_record_detail?sellRecordId='+this.now_id+"&start="+this.modal_start+"&length=10",
				}).then((res) => {
					if(res.status==200){
