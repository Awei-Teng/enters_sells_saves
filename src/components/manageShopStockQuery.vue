<template>
	<div class="box">
		<Breadcrumb class="crumb" separator=">">
			...
			<BreadcrumbItem>门店进货查询</BreadcrumbItem>
		</Breadcrumb>
		<div class="thisName">
			<span>门店进货查询</span>
		</div>
		...
		<div class="main-right">
			<div class="serach">
				...
				<div class="serach_main">
					...
					<span style="font:12px/32px 'microsoft yahei';padding-left:20px">进货状态:</span>
					<Select v-model="tableInfo.arg.stockStatus" style="width:200px">
				        <Option v-for="item in stockStatusList" :value="item.value">{{ item.label }}</Option>
				    </Select>
			...
			<div class="serach" style="margin-top:12px;">
				...
				<div class="serach_main">
					<span style="font:12px/32px 'microsoft yahei';padding-left:20px">进货量:</span>
					<span v-text="countStockNumber" style="color: red;padding-left: 20px;"></span>
					<span style="font:12px/32px 'microsoft yahei';padding-left:40px">实际进货总金额:</span>			
		...
		<Modal width='1000' v-model="modal_detailStoreStockOrder" title="门店进货单详情">
	...
	export default {
		...
		data() {
			return {
				...
				modal_detailStoreStockOrder: false,
				modal_edit: false,
				countStockNumber: '',
				countStockMoney: '',
				data_allprice: {},
            ...
            "detailId": 1,
            "countNumber": 2120,
            "productionDate": "2017-12-07 03:26",
            "brandName": "品牌名称",
            "uTime": "2017-12-07 03:26",
            "avgNmber": 16.7,
            "sellDays": 127.2,
            "laterSellDays": 54.5
        },*/
...
//						}
				...
				tableInfo: {
					...
					url: window.http + '/stock/store_order_list',
					columns: [{
							title: '总公司名称',
							key: 'name',
							align: 'center',
...
//						},
		...
		methods: {
			row_dblclick(row){
				console.log(row)
				this.modal_detailStoreStockOrder = true
				
//				row.storeStockOrderId
				this.$http({
					method: 'get',
					url: window.http + '/stock/store_order_detail?storeStockOrderId='+row.storeStockOrderId,
				}).then((res) => {
					console.log(res)
					if(res.status == 200) {
			...
			down_table(){
				this.$http({
					...
					if(res.status==200){
						...
						for(let i=0;i<res.data.data.length;i++){
							//（1审核2.运输中3.审核不通过4.确认收货）
							if(res.data.data[i].stockStatus == 1){
								res.data.data[i].stockStatus = '1审核2'
							}else if(res.data.data[i].stockStatus == 2){
								res.data.data[i].stockStatus = '运输中3'
							}else if(res.data.data[i].stockStatus == 3){
								res.data.data[i].stockStatus = '审核不通过4'
							}else if(res.data.data[i].stockStatus == 4){
								res.data.data[i].stockStatus = '确认收货'
							}
						}
